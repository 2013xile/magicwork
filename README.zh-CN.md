...
- 总结流程和显式任务动作绑定：`finish` 负责在清理前生成最终总结，`report` 可以在不清理的情况下生成或追加总结。
- 运行时状态尽量保持轻量，只保存 session id，再在生成总结时回到原始 agent 会话源读取内容。
- 使用保存下来的 `sessionId` 回到原始 agent 会话历史，再从这个来源派生总结，这样工作留痕会更可靠。
- 即使 worktree 已经不存在，只要原始 session 记录还在，也应继续基于 agent 会话源生成总结，而不是直接跳过。
...
- `finish` 时自动总结，或者通过 `--no-summary` 显式跳过。
...
`mw clean` 只清理运行态、worktree 和 branch，不会把任务标记为完成。
...
- front matter 是任务配置源
- `status` 使用 `todo | doing | done`
- `status: "done"` 表示任务已经完成，`run` 不会再启动它
- `run` 启动 `todo` 任务时，会自动把它提升为 `doing`
- `prompt` 为空时任务保持 draft，不会被 `run` 启动
- `prompt` 是启动 agent 时发送的初始提示词
- `newBranch` 留空时自动生成
- `hooks` 用来控制该任务启用哪些 lifecycle hook event
- 例如 `hooks: [onClean]` 会跳过启动 hook，但保留清理 hook
- `workspaceRepos` 是一个可选的自定义字段，供你自己的 hook 脚本消费
- `workspaceRepos[].pull` 默认是 `false`，只有显式写 `pull: true` 才会拉取
- `workspaceRepos[].dest` 表示最终目录，不是父目录
- 例如 `dest: ".deps/admin-ui"` 会直接 clone 到 `<task-worktree>/.deps/admin-ui`
...
- `finish` 会生成最终总结、把任务标记为完成，然后再清理工作区
- `clean` 只清理运行态和本地 worktree 资源，不会把任务标记为完成
...
- `hooks.onBeforeLaunch`
  - 在任务 worktree 创建完成后、任何 tab 打开前触发
  - 适合放那些必须先于环境初始化和 prompt 发送完成的前置动作
  - 一个典型例子是按任务元数据里的 `workspaceRepos` 先同步附加仓库
- `hooks.onCodeTabOpen`
  - 在 `run` 时打开 code/setup tab 触发
  - 在 `onBeforeLaunch` 之后执行
...
2. 执行 `onBeforeLaunch`。
3. 打开任务的各个 tab。
4. 在 code/setup tab 里执行 `onCodeTabOpen`。
5. 启动 agent 并发送初始 prompt。
